...
class MessageViewSet(
    ...
    def get_queryset(self):
        ...
        ).select_related("sender", "receiver", "conversation").order_by("-created_at")
    ...
    def perform_create(self, serializer):
        ...
        # Validate conversation existence and participation
        try:
        ...
        # Automatically set receiver (for 1-on-1 chat)
        ...
        notifications = []
        for user in conversation.participants.exclude(id=self.request.user.id):
            # For now, notification body is generic, because message is encrypted
            notifications.append(
                Notification(
                    user=user,
                    notification_type=Notification.TYPE_MESSAGE,
                    title="New message",
                    body="New message",  # could be improved with decrypted preview if needed
